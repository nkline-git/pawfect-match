// US states + DC, standard postal abbreviations — used to disambiguate
// city searches (e.g. "Springfield" exists in a dozen states) so
// geocoding actually finds the right place instead of "location not found"
...
// Best-effort parse of a trailing ", ST" from a freeform city string —
// used to pre-fill the state dropdown from a saved/profile city value
export function parseStateFromCity(cityStr: string): string {
  const match = cityStr.trim().match(/,\s*([A-Za-z]{2})$/)
  if (!match) return ''
  const abbr = match[1].toUpperCase()
  return US_STATES.some(s => s.abbr === abbr) ? abbr : ''
}

// Strip a trailing ", ST" so the city field itself holds just the city name
export function stripStateFromCity(cityStr: string): string {
  return cityStr.trim().replace(/,\s*[A-Za-z]{2}$/, '').trim()
}
